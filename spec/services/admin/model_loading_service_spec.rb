  ...
  context "when #call" do
    ...
    context "when params are present" do
      let!(:search_system_requirements) do
        system_requirements = []
        15.times { |n| system_requirements << create(:system_requirement, name: "Search #{n + 1}", operational_system: "MacOS" ) }
        system_requirements
      end
      
      let!(:unexpected_search) do
        system_requirements = []
      ...
      let(:params) do
        { search: { name: "Search", operational_system: "Mac"}, order: { name: :desc }, page: 2, length: 4 }
      end
      ...
      it "does not return unexpected records" do
        ...
        service = described_class.new(SystemRequirement.all, params)
        service.call
        expect(service.records).to_not include *unexpected_search
      end
    end
